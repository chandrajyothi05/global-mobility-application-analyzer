...
from visa.utils.main_utils import read_yaml_file, write 
...
def initiate_data_validation(self) -> DataValidationArtifact:
    try:
        ...
        logging.info("Starting training validation")
        ...
        if not status:
            validation_error_msg += f"Number of columns in training dataframe does not match the expected number of columns."
        status=self.validate_number_of_columns(dataframe=test_df)
        logging.info(f"All required number of columns present in testing dataframe: {status}")
        if not status:
            validation_error_msg += f"Number of columns in testing dataframe does not match the expected number of columns."
        status=self.is_column_exist(df=train_df)
        if not status:
            validation_error_msg += f"Some columns are missing in training dataframe."
        ...
        if validation_status:
            ...
            logging.info(f"Data validation failed with error: {validation_error_msg}")
