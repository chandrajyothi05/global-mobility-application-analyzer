...
from visa.entity.config_entity import ModelTrainerConfig
from visa.entity.artifact_entity import ModelTrainerArtifact, ClassificationMetricArtifact, DataTransformationArtifact
...
class ModelTrainer:
    ...
    def get_model_object_and_report(self, train: np.array, test: np.array) -> Tuple[object, object]:
        try:
            ...
            logging.info(f"Model metrics — Accuracy: {accuracy}, F1: {f1_score_value}, Precision: {precision}, Recall: {recall}")

            metric_artifact = ClassificationMetricArtifact(f1_score=f1_score_value, precision_score=precision, recall_score=recall)
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        ...
        try:
            ...
            if best_model_detail.best_score < self.model_trainer_config.expected_accuracy:
                logging.info("No best model found with score more than base accuracy")
                raise Exception("No best model found with score more than base accuracy")

            usvisa_model = VisaModel(
