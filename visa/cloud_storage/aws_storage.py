...
class SimpleStorageService:
    ...
    def upload_file(self, from_file_name: str, bucket_name: str, object_name: str=None) -> None:
        logging.info("Entered the upload_file method of S3Operations class")
        try:
            logging.info(
                f"Uploading file: {from_file_name} to {object_name} file in bucket: {bucket_name}"
            )
            self.s3_resource.meta.client.upload_file(
                from_file_name,bucket_name, to_filename
            )
                        
            logging.info(
                f"Uploaded {from_file_name} file to {to_filename} file in bucket: {bucket_name}"
            )
            if remove is True:
                os.remove(from_file_name)
                logging.info(f'Remove is set to {remove}, deleted the file')
            else:
                logging.info(f'Remove is set to {os.remove}, file is not deleted') 
