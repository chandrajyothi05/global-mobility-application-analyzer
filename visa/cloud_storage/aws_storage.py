...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name, s3_key) -> bool:
        try:
            ...
            logging.error(f"Error occurred while checking S3 key path: {e}")
    ...
    def upload_file(self, from_file_name: str, bucket_name: str, to_filename: str, remove: bool = False) -> None:
        logging.info("Entered the upload_file method of S3Operations class")
        try:
            logging.info(f"Uploading file: {from_file_name} to {to_filename} in bucket: {bucket_name}")
            self.s3_resource.meta.client.upload_file(from_file_name, bucket_name, to_filename)
            logging.info(f"Uploaded {from_file_name} to {to_filename} in bucket: {bucket_name}")
            if remove is True:
                os.remove(from_file_name)
                logging.info(f"Remove is set to {remove}, deleted the file")
            else:
                logging.info(f"Remove is set to {remove}, file is not deleted")
    ...
    def upload_df_as_csv(self, data_frame: DataFrame, local_filename: str, bucket_filename: str, bucket_name: str) -> None:
        ...
        try:
            ...
            self.upload_file(local_filename, bucket_name, bucket_filename)
